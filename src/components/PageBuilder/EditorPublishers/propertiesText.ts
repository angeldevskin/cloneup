...
export function EditorPropertiesText(editor: Editor) {
		...
		switch (type) {
			...
			case 'font-size':
				return (
					editor
						.getSelected()
						?.rule?.attributes.style?.['font-size']?.split('px')[0] ?? '18'
				)
			...
			case 'line-height':
				return (
					editor
						.getSelected()
						?.rule?.attributes.style?.['line-height']?.split('px')[0] ?? '18'
				)
	...
	const propertiesTextStyles: Property[] = [
		{
			name: 'EFEITOS',
			property: 'title'
		},
	...
	editor.StyleManager.addSector('editor', {
		name: 'EDITOR',
		open: true,
		properties: propertiesText as PropertyProps[]
	})

	editor.StyleManager.addSector('estilos', {
		name: 'ESTILOS',
		open: false,
		properties: propertiesTextStyles as PropertyProps[]
	})

	editor.on('style:property:update', (style) => {
		if (editor.getSelected()?.attributes.tagName === 'p') {
