...
export function ExternalCodePage({ pagesId, onCloseModal }: any) {
	const navigate = useNavigate()
	const { handleSubmit } = useForm<FormType>()
	const [, setPages] = useState<FunnelType[]>([])
	...
	async function onSubmit() {
		...
		try {
			for (const code of codeToUpdate) {
				const actualPages: string[] = code.pagesIds ?? []
				actualPages.push(pagesId)

				let filteredArray = Array.from(new Set(actualPages))
				if (toRemove.includes(code.id)) {
					filteredArray = filteredArray.filter((f) => f !== pagesId)
				}
				try {
					await updatePixelCodeSourceFunnel(code.id, {
						pagesIds: filteredArray
					})
				} catch (error) {
			...
			setIsPending(false)
			console.error(error)
			toast.error('Erro ao salvar o página.', errorToast)
		}
	}
	...
	function handleIdChange(id: string) {
		...
		const pageExists = externalCodeList?.some((code) => code.platform === id)
		if (!pageExists) {
	...
	useEffect(() => {
		if (externalCodeList) {
			setListCodes(
				externalCodeList?.filter((code) => code.pagesIds?.includes(pagesId))
			)
		}
	}, [externalCodeList])
						...
						<E.ExternalCodeWrapper>
							<div className="footer">
								<label className="footer__title">
									Esta página possui os seguintes códigos:
								</label>
							</div>
